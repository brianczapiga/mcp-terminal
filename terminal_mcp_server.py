...
class TerminalManager:
    ...
    def scan_sessions(self) -> List[SessionInfo]:
        ...
        if current_time - self.last_scan_time < self.scan_interval:
            logger.info(f"Using cached sessions (scanned {current_time - self.last_scan_time:.1f}s ago)")
    ...
    def send_keypress(self, session_id: str, key: str, modifiers: Optional[List[str]] = None) -> bool:
...
class SendInputRequest(BaseModel):
    ...
    execute: bool = Field(default=True, description="Whether to execute the command (press Enter)")
    session_id: Optional[str] = Field(None, description="Session ID to send to (uses active if not specified)")


class SendKeypressRequest(BaseModel):
    key: str = Field(..., description="Key to press (e.g., 'return', 'tab', 'escape', 'up', 'down', 'left', 'right')")
    modifiers: Optional[List[str]] = Field(default=None, description="Modifier keys (e.g., ['command', 'shift'])")
    session_id: Optional[str] = Field(None, description="Session ID to send to (uses active if not specified)")
...
class ScrollBackRequest(BaseModel):
    ...
    session_id: Optional[str] = Field(None, description="Session ID to scroll back (uses active if not specified)")
...
async def terminal_session_summary(session_id: str, include_history: bool = False) -> str:
...
@server.tool()
async def set_active_session(request: SetActiveSessionRequest) -> Dict[str, Union[bool, str]]:
    ...
    try:
        ...
        return {
            "success": success,
            "message": f"Active session set to {request.session_id}" if success else f"Session {request.session_id} not found"
        }
    except Exception as e:
...
@server.tool()
async def get_screen(request: GetScreenRequest) -> Dict[str, Union[str, List[Dict[str, str]]]]:
    ...
    try:
        if request.mode == "recent-output":
            ...
            if recent_tty:
                ...
                for session in sessions:
                    if session.tty_device == recent_tty:
                        ...
                        return {"mode": "recent-output", "content": content, "session_id": session_id}
            ...
            if terminal_manager.active_session_id:
                ...
                return {"mode": "focus", "content": content, "session_id": terminal_manager.active_session_id}
            else:
...
async def send_input(request: SendInputRequest) -> Dict[str, Union[bool, str]]:
    ...
    try:
        ...
        if is_readonly_mode():
            logger.warning("Input injection blocked: Server is running in readonly mode")
            return {
                "success": False, 
        ...
        return {
            "success": success,
            "message": f"Input sent to session {session_id}" if success else f"Failed to send input to session {session_id}"
        }
    except Exception as e:
...
async def send_keypress(request: SendKeypressRequest) -> Dict[str, Union[bool, str]]:
    ...
    try:
        # Security: Check readonly mode
        if is_readonly_mode():
            logger.warning("Keypress injection blocked: Server is running in readonly mode")
            return {
                "success": False, 
                "message": "Input injection is disabled. Set MCP_TERMINAL_READONLY=0 to enable."
            }
        ...
        success = terminal_manager.send_keypress(session_id, request.key, request.modifiers)
        return {
            "success": success,
            "message": f"Keypress sent to session {session_id}" if success else f"Failed to send keypress to session {session_id}"
        }
    except Exception as e:
...
async def paste_text(request: PasteTextRequest) -> Dict[str, Union[bool, str]]:
    ...
    try:
        ...
        return {
            "success": success,
            "message": f"Text pasted to session {session_id}" if success else f"Failed to paste text to session {session_id}"
        }
    except Exception as e:
